...
\caption{A 3-dimensional depiction of the radiation pattern for a dipole antenna}
\end{figure}
...
Figure 2.20 shows that the dipole antenna is a omnidirectional antenna, meaning it is radiating equally in all direction in the horizontal plane, whereas figure 2.22 and 2.23 shows the radiation pattern of a helix antenna, where the radiation is focused towards a point. What can also be seen on figure 2.22 and 2.23 is the side loops around the main loop (the single peak direction), this means that not all of the energy is emitted "through" the main loop, but some goes to the side loop, which make the antenna less effective, see section 2.6.3 \textit{Helical} for more information about directed antennas. 

\begin{figure}[h]
\centering
%\hspace{-1cm}
\includegraphics[scale=0.45]{figures/HelixRad.PNG}
\caption{Radiation pattern for a helix antenna }
\end{figure}
...
The far-field region is the most important region, as this determine the antenna's radiation pattern, described in the section above. Antennas are use to communicate wirelessly from long distances, so this is the region of operation for most antennas. The far-field region is far from the antenna, as one might suspect. In this region, the radiation pattern does not change shape with distance (although the fields still die off as $\frac{1}{r}$). This region is dominated by the radiated fields, with E- and H-planes being orthogonal to each other. The far-field region is defined for distances $r$, such that 3 condition must be satisfied:
...
The radiating near-field region is also called the Fresnel region, since it is between the reactive near-field region and the far-field region. It is defined as:

\begin{equation}
    0.62 \cdot \sqrt{\frac{D^3}{\lambda}} < r < \frac{2D^2}{\lambda}
\end{equation}

Unlike far-field region the shape in the radiating near-field 
